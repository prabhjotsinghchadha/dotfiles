...
function handleLinterJson(currentSourceName, output, config, fpath) {
    if (!config.parseJson) {
        throw new Error('missing parseJson');
    }
    var _a = config.parseJson, sourceName = _a.sourceName, line = _a.line, column = _a.column, endLine = _a.endLine, endColumn = _a.endColumn, security = _a.security, message = _a.message;
    var errorsRoot = (typeof config.parseJson.errorsRoot === 'string')
        ? config.parseJson.errorsRoot.replace('%filepath', fpath)
        : config.parseJson.errorsRoot.map(function (v) { return v.replace('%filepath', fpath); });
    var resultsFromJson = errorsRoot
        ? get_1.default(JSON.parse(output), errorsRoot, [])
        : JSON.parse(output);
    return resultsFromJson.filter(function (jsonObject) {
        return null !== get_1.default(jsonObject, line);
...
function handleLinter(textDocument, config) {
    return tslib_1.__awaiter(this, void 0, void 0, function () {
        var command, _a, rootPatterns, _b, args, _c, offsetLine, _d, offsetColumn, _e, offsetLineStart, _f, offsetLineEnd, _g, offsetColumnStart, _h, offsetColumnEnd, sourceName, isStdout, isStderr, ignore, _j, securities, diagnostics, currentFile_1, workDir_1, relPath, cmd, output, _k, _l, stdout, _m, stderr, linterResults, lengthBefore, error_1;
        return tslib_1.__generator(this, function (_o) {
            switch (_o.label) {
                case 0:
                    command = config.command, _a = config.rootPatterns, rootPatterns = _a === void 0 ? [] : _a, _b = config.args, args = _b === void 0 ? [] : _b, _c = config.offsetLine, offsetLine = _c === void 0 ? 0 : _c, _d = config.offsetColumn, offsetColumn = _d === void 0 ? 0 : _d, _e = config.offsetLineStart, offsetLineStart = _e === void 0 ? 0 : _e, _f = config.offsetLineEnd, offsetLineEnd = _f === void 0 ? 0 : _f, _g = config.offsetColumnStart, offsetColumnStart = _g === void 0 ? 0 : _g, _h = config.offsetColumnEnd, offsetColumnEnd = _h === void 0 ? 0 : _h, sourceName = config.sourceName, isStdout = config.isStdout, isStderr = config.isStderr, ignore = config.ignore, _j = config.securities, securities = _j === void 0 ? {} : _j;
                    diagnostics = [];
                    // verify params
                ...
                case 2:
                    workDir_1 = _o.sent();
                    relPath = path_1.relative(workDir_1, currentFile_1);
                    try {
                ...
                case 4:
                    _k = _o.sent(), _l = _k.stdout, stdout = _l === void 0 ? '' : _l, _m = _k.stderr, stderr = _m === void 0 ? '' : _m;
                    ...
                    linterResults = config.parseJson
                        ? handleLinterJson(sourceName, output, config, vscode_uri_1.URI.parse(textDocument.uri).fsPath)
